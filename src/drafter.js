...
Write a warm, professional thank-you email in Richard Daniel's voice:
...
Keep it under 180 words total. Do not be sycophantic or use hollow phrases.`,
  ...
  // Save to Gmail drafts — Richard reviews and sends manually
 ...
 */
export async function draftOutreach({ name, title, company, context, type = 'email' }) {
  console.log(`[Drafter] Drafting ${type} outreach to ${name} at ${company}...`);

  const typeInstructions = {
    email: 'Write a professional email. Include subject line as SUBJECT: <subject>.',
    linkedin: 'Write a LinkedIn connection message. Max 300 characters. No subject line needed.',
    'referral-ask': 'Write a warm message asking for a referral introduction. Friendly tone, acknowledge the relationship.',
  };
  ...
  const response = await anthropic.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `Draft a ${type} outreach message from Richard Daniel to:
...
Guidelines:
- Write in Richard's voice: direct, warm, not desperate
